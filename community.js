...
function setupDraggableButton(buttonId, dragRight) {
    ...
    document.addEventListener('mousemove', function(e) {
        ...
        if (isDragging) {
            ...
            let adjustedTranslateX = dragRight ? Math.min(Math.max(translateX + initialX, maxLeft), maxRight) - initialX : Math.max(Math.min(translateX + initialX, maxRight), maxLeft) - initialX;
